...
from services.parser.cert_txt_parser import parse_cert_y1_text
...
# ---------------------------------------------------------------------------
# Импорт Y1 через спец-парсер
# ---------------------------------------------------------------------------

@_require_user
async def import_y1(request: web.Request) -> web.Response:
    """Принимает либо {"text": "..."} JSON, либо multipart с файлом (txt/docx/pdf)."""
    variant_id = int(request.match_info["variant_id"])
    ...
    drafts = parse_cert_y1_text(text)

    async with AsyncSessionFactory() as db:
        async with db.begin():
            created = await cs.import_y1_drafts(db, request["user_id"], variant_id, drafts)
        added = len(created)
