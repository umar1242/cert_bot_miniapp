          ...
          <div className="sheet-section" key={qtype}>
            ...
            <div className="sheet-grid">
              {cells.map((n) => {
                ...
                if (!q && qtype !== "Y1") classes.push("add");

                return (
                  <button
                    key={n}
                    className={classes.join(" ")}
                    onClick={() => (q ? onSelectQuestion(q) : qtype !== "Y1" && onAddInSection(qtype))}
                    title={q ? q.text.slice(0, 60) : qtype === "Y1" ? "Импортируйте через парсер выше" : "Добавить задание"}
                  >
                    {n}
                  </button>
                );
