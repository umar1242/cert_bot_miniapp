...
async def import_y1_drafts(
    ...
    """Добавляет распарсенные задания Y1 в вариант, начиная со свободного номера 1..32."""
    variant = await get_variant(db, owner_id, variant_id)
    if not variant:
...
async def add_manual_question(db: AsyncSession, owner_id: int, variant_id: int, payload: dict) -> CertQuestion:
    """Добавляет задание Y2/O1/O2, созданное вручную в интерфейсе mini app."""
    variant = await get_variant(db, owner_id, variant_id)
    if not variant:
        raise ValueError("Вариант не найден")

    qtype = payload.get("qtype")
    ranges = {"Y2": Y2_RANGE, "O1": O1_RANGE, "O2": O2_RANGE}
    ...
    if qtype == "Y2":
        for i, pair in enumerate(payload.get("pairs", [])):
            db.add(CertMatchPair(
                ...
                left_text=pair.get("left", ""), right_text=pair.get("right", ""),
            ))
    elif qtype == "O1":
...
async def update_question(db: AsyncSession, owner_id: int, question_id: int, payload: dict) -> CertQuestion:
    ...
    if question.qtype == CertQType.Y2 and "pairs" in payload:
        for p in list(question.match_pairs):
            await db.delete(p)
        await db.flush()
        for i, pair in enumerate(payload["pairs"]):
            db.add(CertMatchPair(question_id=question.id, position=i, left_text=pair.get("left", ""), right_text=pair.get("right", "")))
...
def _progress(variant: CertVariant) -> dict:
    ...
    for q in variant.questions:
        ...
        if q.needs_image:
            needs_image += 1
    total_slots = (Y1_RANGE[1] - Y1_RANGE[0] + 1) + (Y2_RANGE[1] - Y2_RANGE[0] + 1) + (O1_RANGE[1] - O1_RANGE[0] + 1) + (O2_RANGE[1] - O2_RANGE[0] + 1)
